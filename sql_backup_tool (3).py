...
def run_backup(table_name: str, app: "BackupApp"):
    """
    Ejecuta el backup usando BCP (Bulk Copy Program) — el mismo motor
    que usa el wizard de SQL Server. Dramatically más rápido que fetchmany.
    Fallback a pyodbc si bcp no está disponible.
    """
    import subprocess
    import shutil
    ...
    # ── Intentar con BCP primero ──────────────────────────────────────────────
    bcp_path = shutil.which("bcp")
    if bcp_path:
        app.log(f"⚡ Usando BCP (modo rápido): {bcp_path}")
        _run_with_bcp(table_name, filepath, app)
    else:
        app.log("⚠  BCP no encontrado en PATH. Usando pyodbc (modo lento).")
        app.log("   Para activar el modo rápido instala SQL Server Command Line Utilities:")
        app.log("   https://learn.microsoft.com/sql/tools/bcp-utility")
        app.log("─" * 60)
        _run_with_pyodbc(table_name, filepath, app)


def _run_with_bcp(table_name: str, filepath: str, app: "BackupApp"):
    """Exporta usando bcp queryout — el método más rápido disponible."""
    import subprocess
    ...
    cmd = [
        ...
        "-S", SERVER,
        "-d", DATABASE,
        "-T",               # Windows Auth; reemplazar por -U user -P pass si es SQL Auth
        "-c",               # Formato carácter (texto plano)
        "-t", SEPARATOR,    # Separador de columnas
        "-r", "\n",         # Separador de filas
        "-b", str(CHUNK_SIZE),  # Tamaño de lote para commit
    ]

    # Si usas SQL Auth, comenta -T y descomenta esto:
    # cmd += ["-U", "TU_USUARIO", "-P", "TU_PASSWORD"]

    app.log(f"   Comando: {' '.join(cmd)}")
    app.log("   Exportando... (el progreso lo reporta BCP cada lote)")
    app.log("─" * 60)
    ...
    try:
        ...
        # Leer salida de BCP en tiempo real y mostrarla en el log
        for line in proc.stdout:
            line = line.strip()
            if line:
                app.log(f"   {line}")
                # BCP reporta "X rows copied." — extraemos el número para la barra
                if "rows copied" in line.lower():
                    try:
                        rows = int(line.strip().split()[0].replace(",", ""))
                        app.set_progress_bcp(rows)
                    except Exception:
                        pass
        ...
        if proc.returncode == 0:
            ...
            app.log("─" * 60)
            app.log(f"✅ Backup completado exitosamente (BCP).")
            app.log(f"   Tiempo total   : {format_time(elapsed)}")
            app.log(f"   Tamaño archivo : {size_mb:.2f} MB")
            app.log(f"   Guardado en    : {filepath}")
            app.set_state("done", success=True)
        else:
            app.log(f"❌ BCP terminó con código de error {proc.returncode}.")
            app.set_state("done", success=False)

    except Exception as e:
        app.log(f"❌ Error ejecutando BCP: {e}")
        app.set_state("done", success=False)


def _run_with_pyodbc(table_name: str, filepath: str, app: "BackupApp"):
    """Fallback: exporta leyendo por chunks con pyodbc."""
    try:
        ...
        with open(filepath, "w", encoding="utf-8", buffering=1024 * 1024) as f:
            ...
            while True:
                ...
                if time.time() - last_log >= 30:
                    ...
                    app.log(f"   ⏳ {rows_written:,} / {total_rows:,} ({pct:.1f}%)  ETA: {format_time(remaining)}")
                    last_log = time.time()

        elapsed_total = time.time() - start_time
        size_mb = os.path.getsize(filepath) / (1024 * 1024)
        app.log("─" * 60)
        app.log(f"✅ Backup completado. {rows_written:,} filas | {format_time(elapsed_total)} | {size_mb:.2f} MB")
        app.log(f"   Guardado en: {filepath}")
        cursor.close()
        conn.close()
        app.set_state("done", success=True)
        ...
        app.log(f"❌ Error de base de datos: {e}")
        app.set_state("done", success=False)
    except Exception as e:
...
class BackupApp:
    ...
    def set_progress_bcp(self, rows_done: int):
        """BCP no conoce el total, muestra barra indeterminada con contador."""
        def _update():
            self._lbl_rows.config(text=f"{rows_done:,} filas copiadas")
            # Animación de pulso en la barra (avanza en ciclos de 0-100)
            current = self._progressvar.get()
            self._progressvar.set((current + 5) % 101)
